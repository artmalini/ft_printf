...
static void	*ft_memalloc(int size)
{
	char	*v;
	int	i;
...
static char	*ft_strncpy(char *dst, const char *src, int len) // Мой с этим не работает
{
	char *d;
...
char 	*build_str_char(va_list arg, t_bone *elem)
{
	...
	{
		if (elem->precis >= 0)
			str = utf_char_len_concat(va_arg(arg, wchar_t*), (int)elem->precis);
		else
	...
	else
	{
		//printf("n %zu\n", (int)elem->precis);
		if (elem->precis >= 0)
			str = ft_strlendup(va_arg(arg, char*), (int)elem->precis);
		else
...
int 		print_str_with_flags(char *str, t_bone *elem, int str_len)
{
	...
	//prf_print_str_ln(str, ft_strlen(str)); ////print numbers
